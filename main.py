...
load_dotenv()
db = None
_initial_fb_logger = logging.getLogger("FIREBASE_INIT_LOGGER_UNIQUE_NAME_V2") # نام جدید برای جلوگیری از تداخل
...
# POINTS_FOR_PROFILE_COMPLETION (فقط سن و جنسیت) حذف شد
# POINTS_FOR_NAME_COMPLETION (فقط نام) حذف شد
# POINTS_FOR_CLUB_TIP حذف شد

BADGE_CLUB_MEMBER = "عضو باشگاه تافته 🏅"
# BADGE_PROFILE_COMPLETE (فقط سن و جنسیت) حذف شد یا تغییر نام می‌دهد
BADGE_FULL_PROFILE = "پروفایل طلایی ✨" # برای نام، نام خانوادگی، سن و جنسیت
...
# --- کیبوردها (بدون تغییر نسبت به آخرین نسخه کامل ارسالی) ---
...
def get_or_create_user_profile(user_id: str, username: str = None, first_name: str = None) -> dict:
    if not db:
        logger.warning(f"DB Firestore None. Mock profile for user {user_id}.")
    ...
    try: user_doc = user_ref.get()
        ...
        logger.error(f"DB: خطا get() برای {user_id}: {e}", exc_info=True)
        return {"user_id": user_id, "username": username, "first_name": first_name, "age": None, "gender": None, "is_club_member": False, "points": 0, "badges": [], "club_tip_usage_count": 0, "club_join_date": None, "name_first_db": None, "name_last_db": None, "full_profile_completion_points_awarded": False}
    ...
    if user_doc.exists:
        ...
        for key, val in default_fields.items():
            if key not in user_data: user_data[key] = val; needs_update = True
        if needs_update:
            update_payload = {k:v for k,v in default_fields.items() if k not in user_doc.to_dict()}
            if update_payload:
                try: user_ref.update(update_payload)
                except Exception as e_upd: logger.error(f"DB: خطا آپدیت فیلد پیش‌فرض برای {user_id}: {e_upd}")
        return user_data
    else:
        ...
        except Exception as e_set: logger.error(f"DB: خطا ایجاد پروفایل جدید برای {user_id}: {e_set}")
        return user_data
...
def _prepare_doctor_system_prompt(age: int, gender: str) -> str:
    # ... (همان پرامپت کوتاه و متمرکز شده قبلی) ...
    return (
...
async def get_dynamic_main_menu_keyboard(context: ContextTypes.DEFAULT_TYPE, user_id_str: str) -> ReplyKeyboardMarkup:
    # ... (بدون تغییر) ...
    is_member = False
    if 'is_club_member_cached' in context.user_data: del context.user_data['is_club_member_cached'] # همیشه از نو بخوان
    if db:
        try:
            user_profile = await asyncio.to_thread(get_user_profile_data, user_id_str)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> States:
    ...
    message_prefix = "درخواست لغو انجام شد. " if context.user_data.get('_is_cancel_flow', False) else ""
    if context.user_data.get('_is_cancel_flow', False): del context.user_data['_is_cancel_flow']
    logger.info(f"کاربر {user_id_str} ({user.full_name or user.username}) /start یا بازگشت.")
    keys_to_clear = ["doctor_chat_history", "system_prompt_for_doctor", "age_temp", "is_club_member_cached", "awaiting_field_to_edit", "temp_first_name", "profile_completion_flow_active", "club_join_after_profile_flow", "temp_profile_first_name", "temp_profile_last_name", "temp_profile_age"]
    ...
    try:
        ...
        if is_direct_start and not (hasattr(update.message, 'photo') and update.message.photo): 
...
async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE) -> States:
    ...
    logger.info(f"User {user_id} /cancel. Delegating to start.")
    context.user_data['_is_cancel_flow'] = True
    if update.effective_chat: await context.bot.send_message(chat_id=update.effective_chat.id, text="درخواست لغو شد.", reply_markup=ReplyKeyboardRemove())
...
async def main_menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> States:
    ...
    if text == "👨‍⚕️ دکتر تافته":
        ...
        if age and gender and name_first and name_last: 
            ...
            await update.message.reply_text("برای استفاده از دکتر تافته، لطفاً پروفایل خود را تکمیل کنید.\nنام کوچک خود را وارد کنید:", reply_markup=PROFILE_INPUT_BACK_KEYBOARD)
        ...
        if not (age and gender and name_first and name_last): 
            ...
            await update.message.reply_text("برای عضویت، ابتدا پروفایل خود را تکمیل کنید.\nنام کوچک خود را وارد کنید:", reply_markup=PROFILE_INPUT_BACK_KEYBOARD)
            return States.AWAITING_PROFILE_FIRST_NAME
        else: 
            await update.message.reply_text("آیا مایل به عضویت در باشگاه مشتریان هستید؟", reply_markup=CLUB_JOIN_CONFIRMATION_KEYBOARD)
...
# --- کنترل‌کننده‌های جریان تکمیل پروفایل کامل (AWAITING_PROFILE_...) ---
async def awaiting_profile_first_name_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> States:
    ...
    await update.message.reply_text("نام خانوادگی خود را وارد کنید (یا بازگردید):", reply_markup=PROFILE_INPUT_BACK_KEYBOARD)
...
async def awaiting_profile_age_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> States:
    ...
    if not text.isdigit() or not (1 <= int(text) <= 120):
        await update.message.reply_text("سن معتبر (عدد ۱-۱۲۰) وارد کنید یا بازگردید.", reply_markup=PROFILE_INPUT_BACK_KEYBOARD)
        return States.AWAITING_PROFILE_AGE
...
async def awaiting_profile_gender_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> States:
    ...
    gender = gender_input
    awarded_full_profile_badge = False
    if db:
        try:
            ...
            if awarded_full_profile_badge: logger.info(f"کاربر {user_id_str} واجد شرایط امتیاز و نشان پروفایل کامل است.")
    ...
    if context.user_data.pop('club_join_after_profile_flow', False):
        await update.message.reply_text("اکنون برای عضویت در باشگاه تایید نهایی را انجام دهید:", reply_markup=CLUB_JOIN_CONFIRMATION_KEYBOARD)
        return States.AWAITING_CLUB_JOIN_CONFIRMATION
...
async def handle_club_join_confirmation(update: Update, context: ContextTypes.DEFAULT_TYPE) -> States:
    # ... (بدون تغییر عمده، فقط reply_markup ها در انتها) ...
...
async def doctor_conversation_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> States:
    ...
    if not system_prompt:
        logger.warning(f"DCH: System prompt for user {user_id_str} not found! Rebuilding.")
        ...
        if age_db and gender_db and name_f and name_l: # نیاز به پروفایل کامل برای دکتر
            system_prompt = _prepare_doctor_system_prompt(age_db, gender_db)
            context.user_data["system_prompt_for_doctor"] = system_prompt
            logger.info(f"DCH: System prompt for {user_id_str} rebuilt.")
        else:
            logger.error(f"DCH: Could not rebuild system prompt for {user_id_str}. Profile incomplete. Redirecting to profile completion.")
    ...
    if user_question == "🔙 بازگشت به منوی اصلی": return await start(update, context)
        ...
        context.user_data["doctor_chat_history"] = []
        await update.message.reply_text("تاریخچه پاک شد. سوال جدید:", reply_markup=DOCTOR_CONVERSATION_KEYBOARD)
        return States.DOCTOR_CONVERSATION
...
async def profile_view_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> States:
    ...
    if text == "💔 لغو عضویت از باشگاه":
        await update.message.reply_text("مطمئن هستید؟\n⚠️ **اخطار:** امتیازات، نشان‌ها و اطلاعات پروفایل (سن، جنسیت، نام) ریست خواهد شد.", reply_markup=CANCEL_MEMBERSHIP_CONFIRMATION_KEYBOARD, parse_mode="Markdown")
...
async def edit_last_name_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> States: # قبلا awaiting_last_name_handler
    # ... (با اعطای امتیاز تکمیل نام کامل فقط اگر قبلا داده نشده و پروفایل پایه هم کامل بوده) ...
    ...
    if not first_name: return await my_profile_info_handler(update, context) # خطا، بازگشت به پروفایل
        ...
        try:
            ...
            if user_profile_before and \
               ...
               user_profile_before.get('age') and user_profile_before.get('gender'): # اگر سن و جنسیت هم از قبل کامل بوده
                update_payload["points"] = firestore.Increment(POINTS_FOR_FULL_PROFILE_COMPLETION) # استفاده از امتیاز جدید
                update_payload["full_profile_completion_points_awarded"] = True
                awarded_full_profile_badge = True
...
async def health_tip_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> States:
    ...
    try:
        ...
        if user_profile and user_profile.get('is_club_member'):
            tip_system_prompt = ("یک متخصص سلامت هستید. یک نکته سلامتی کوتاه (۱-۲ جمله)، مفید، علمی و کاربردی به فارسی ارائه دهید. "
                                 "نکته عمومی باشد. پاسخ فقط خود نکته باشد، بدون مقدمه.")
            health_tip = await ask_openrouter(tip_system_prompt, [{"role": "user", "content": "نکته سلامتی"}])
...
if __name__ == '__main__':
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            States.AWAITING_DOCTOR_AGE: [MessageHandler(filters.Regex("^🔙 بازگشت به منوی اصلی$"), start), MessageHandler(filters.TEXT & ~filters.COMMAND, request_doctor_age_handler)], # این حالت برای زمانی است که فقط سن و جنسیت برای دکتر لازم است
    ...
    telegram_application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, fallback_message)) # اطمینان از تعریف fallback_message
