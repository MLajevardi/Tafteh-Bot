...
OPENROUTER_MODEL_NAME = os.getenv("OPENROUTER_MODEL_NAME", "openai/gpt-3.5-turbo") # یا مدلی که برای مکالمه بهتر است
...
async def ask_openrouter(system_prompt: str, chat_history: list) -> str:
    ...
    body = {
        ...
        # "temperature": 0.7, # می‌توانید برای خلاقیت بیشتر یا کمتر، این پارامتر را تنظیم کنید
        # "max_tokens": 1000, # می‌توانید حداکثر طول پاسخ را محدود کنید
    }
    ...
    async with httpx.AsyncClient(timeout=90.0) as client:
        try:
            ...
            if data.get("choices") and len(data["choices"]) > 0 and data["choices"][0].get("message") and data["choices"][0]["message"].get("content"):
                ...
                # این بخش برای تحلیل نوع پاسخ LLM (سوال یا جواب) است، می‌توانید گسترش دهید
                if "?" in llm_response_content or \
                   any(phrase in llm_response_content for phrase in ["بیشتر توضیح دهید", "آیا علامت دیگری دارید", "چه مدت است که", "چگونه است"]):
...
def _prepare_doctor_system_prompt(age: int, gender: str) -> str:
    ...
    return (
        ...
        "وظیفه شما این است که از طریق یک مکالمه چند مرحله‌ای با کاربر، به سوالات پزشکی او به زبان فارسی روان، صحیح و قابل فهم برای عموم پاسخ دهید. "
        "**در انتخاب واژگان و ساختار جملات فارسی بسیار دقت کنید. از اصطلاحات و واژگان صحیح و رایج پزشکی و عمومی در زبان فارسی استفاده کنید. از به کار بردن عبارات نامفهوم، گنگ، با ترجمه تحت‌اللفظی ضعیف یا ساختار جملات غیرطبیعی در فارسی جداً خودداری کنید.** "

        "وقتی کاربر سوال اولیه‌ای مطرح می‌کند، ابتدا فکر کنید آیا برای دادن یک پاسخ کامل، دقیق و ایمن نیاز به دانستن زمینه یا شرایط خاصی از کاربر دارید یا خیر (مثلاً آیا بیماری خاصی دارد، داروی خاصی مصرف می‌کند، یا جزئیات بیشتری در مورد سوالش نیاز است). **اگر چنین بود، ابتدا آن سوالات تکمیلی را بپرسید (همچنان بسیار مهم: در هر نوبت فقط یک، و حداکثر دو سوال کوتاه، کلیدی و کاملاً مرتبط بپرسید). از پرسیدن لیست بلند سوالات یا سوالات متعدد در یک پیام جداً خودداری کنید.** "

        "**وقتی کاربر در مورد مضرات یک چیز سوال می‌کند (مثلاً 'آیا فلان چیز ضرر دارد؟')، پاسخ خود را با احتیاط و با در نظر گرفتن تمام جوانب ارائه دهید. از دادن پاسخ‌های قطعی و ساده‌انگارانه که ممکن است در برخی شرایط نادرست باشند، پرهیز کنید. در صورت لزوم، شرایطی که آن چیز می‌تواند مضر باشد و شرایطی که مفید است را به طور متوازن توضیح دهید.** "
        ...
        "پس از چند نوبت پرسش و پاسخ، و **تنها زمانی که مطمئن شدید اطلاعات کافی برای یک توصیه جامع و ایمن در اختیار دارید، توصیه نهایی خود را ارائه دهید.** پس از ارائه توصیه نهایی، می‌توانید از کاربر بپرسید آیا سوال دیگری دارد یا نیاز به شفاف‌سازی بیشتری هست. "
...
async def request_gender_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> States:
    ...
    age = context.user_data.get("age")
    gender = context.user_data.get("gender") # gender_input را اینجا استفاده می‌کنیم
...
async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE) -> States:
    ...
    await update.message.reply_text("درخواست شما لغو شد. بازگشت به منوی اصلی...", reply_markup=ReplyKeyboardRemove()) # کیبورد را حذف می‌کنیم قبل از نمایش منوی اصلی توسط start
    return await start(update, context) 
